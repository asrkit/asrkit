...
| **连装都想省(临时跑一次)** | `uvx asrkit transcribe audio.wav -m siliconflow/sensevoice --api-key <KEY>` —— 从 PyPI 拉起即跑,不落地 |
| **常规 Python 用户** | `pip install asrkit` |
| **要全局命令、又不污染当前环境** | `pipx install asrkit` |
| **要用端侧引擎(sherpa,47 模型)** | `pip install "asrkit[local]"` —— 本地引擎跑在你的 Python 环境里 |

> `uv` 装法:`curl -LsSf https://astral.sh/uv/install.sh | sh`(macOS/Linux 一行,单二进制、零运行时)。
...
asrkit run sherpa/sensevoice audio.wav                     # 端侧:首次自动下模型,然后识别(需 asrkit[local])
```
...
```bash
asrkit stream sherpa/paraformer-online meeting.wav             # live 增量 → stderr,最终文本 → stdout
asrkit stream sherpa/paraformer-online meeting.wav > out.txt   # 管道友好:只截获最终文本
asrkit stream sherpa/sensevoice --mic                          # 麦克风实时转写,Ctrl-C 停止(需 asrkit[mic])
```
...
| 引擎 | 安装 | 寻址 | 覆盖 |
|---|---|---|---|
| **sherpa-onnx**(默认端侧) | `asrkit[local]` | `sherpa/<模型>` 或裸名 `<模型>` | 47 端侧模型,17 架构 |
