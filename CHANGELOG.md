...
- **cloud-only 运行入口**：新增未来 `asrkitd` 使用的内部构建入口；进程启动前将 registry 锁定为 `cloud` profile，只加载 10 个内置云模型和对应 adapter，明确跳过本地引擎、用户模型与第三方插件。完整 Python wheel 只安装 `asrkit`，不占用未来独立产物的 `asrkitd` 命令；自包含二进制和 embedded 生命周期尚未实现。
