...
def test_multilingual_marked():
    for mid in ["sherpa/whisper-tiny", "sherpa/omnilingual-300m", "sherpa/qwen3-asr-0.6b",
                "sherpa/funasr-nano", "sherpa/dolphin-small",
...
def test_multilingual_not_marked():
    for mid in ["sherpa/whisper-tiny-en", "sherpa/moonshine-tiny",
                "faster-whisper/distil-large-v3", "sherpa/paraformer-zh"]:
        assert not _caps(mid).get("multilingual"), mid


def test_sensevoice_precise_langs_no_flag():
    m = registry.resolve("sherpa/sensevoice")
...
def test_list_lang_multilingual_and_explicit(capsys):
    ...
    assert "sherpa/whisper-tiny" in ids          # multilingual flag
    assert "sherpa/sensevoice" in ids            # 显式 ja
    assert "sherpa/whisper-tiny-en" not in ids   # en-only
    assert "sherpa/paraformer-zh" not in ids     # zh-only 非多语


def test_list_lang_normalizes_case(capsys):
    _, out = _run(["list", "--lang", "YUE", "--source", "local", "--json"], capsys)
    ids = {d["id"] for d in _json.loads(out)}
    assert "sherpa/sensevoice" in ids            # 归一化大写
...
def test_list_arch_case_insensitive(capsys):
    ...
    assert ids1 == ids2 and "sherpa/sensevoice" in ids1
...
def test_search_matches_id_name(capsys):
    ...
    assert any(i.startswith("sherpa/whisper") for i in ids)
...
def test_show_multilingual_line(capsys):
    _, out = _run(["show", "sherpa/whisper-tiny"], capsys)
    assert "multilingual: yes" in out
    _, out2 = _run(["show", "sherpa/paraformer-zh"], capsys)
