...
> 状态:**目标设计 + 0.5.5 Python 模块契约**(2026-07-23)。cloud-only profile、embedded 生命周期、网关安全边界、macOS arm64 原型和 Linux x64 无 Python Debian 验证已完成；自包含二进制资产、真实云转写、其余平台发行和 npm 入口仍未完成。
> 北极星与边界见 [product-form.md](product-form.md);本文只讨论云端网关的分发和集成,不改变本地引擎仍以 Python 侧为主的决定。
> 已发布的 0.5.5 Python 包同时包含 `asrkit serve` 与 `python -m asrkit.daemon`。`profiles/` 与 `daemon/` 已物理分开,后者实现只含 10 个云模型的进程 profile、embedded 握手、Bearer 鉴权、资源限制和父进程监控；完整 Python wheel 仍不安装 `asrkit-cloud` 命令。仓库可构建自包含原型,但尚无正式发布的二进制资产。当前 HTTP 子集见 [openai-compatibility.md](openai-compatibility.md)。
...
0.5.5 daemon 模块启动成功后,stdout 输出一条 NDJSON 控制消息:

```json
{"event":"ready","base_url":"http://127.0.0.1:52137/v1","pid":23456,"protocol_version":1}
```

`protocol_version: 1` 是 0.5.5 daemon 模块的 embedded 协议版本；自包含二进制发布前仍不构成独立二进制的兼容承诺。
...
0.5.5 daemon 模块默认值为单次上传 200 MiB、最多 4 个活动转写、转写请求 300 秒、优雅关停 10 秒；CLI 可在受控范围内调整。并发超额立即返回 429,上传超额返回 413,转写超时返回 504。同步厂商调用超时后,临时文件和并发槽会保留到后台调用真正结束,避免仍在读取的音频被提前删除。

已发布的 `asrkit serve` 仍只应描述为受信任本机服务,不应宣传为公网生产网关；上述鉴权和资源默认值属于 0.5.5 daemon 模块及未来 `asrkit-cloud` 二进制入口。
...
Sidecar 版本随宿主产品钉死和升级,不得自行静默更新。0.5.5 daemon 模块的健康端点返回:
...
以上扩展字段属于已发布的 0.5.5 daemon 模块契约；`asrkit serve` 的 `/health` 仍只返回 `{"status":"ok"}`。自包含 `asrkit-cloud` 二进制尚未作为发行资产发布。
...
1. **已完成（0.5.5 Python 模块）**:抽出 `full/cloud` 加载 profile 和 `asrkit-cloud` 内部构建入口,完整 wheel 只安装 `asrkit`;
2. **已完成（0.5.5 Python 模块）**:实现 `--embedded --port 0`、ready/shutdown NDJSON、信号退出和父进程监控;
3. **已完成（0.5.5 Python 模块）**:强制 loopback、宿主随机 token、私有 data dir、上传/并发/请求/关停边界;
