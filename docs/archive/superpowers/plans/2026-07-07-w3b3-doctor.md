...
git -c user.name="BolynWang" -c user.email="1710998763@qq.com" commit -m "feat(doctor): diagnose 离线体检(试写探针/config 直读/密钥只报 vendor)+ --net 探针"
```
...
git -c user.name="BolynWang" -c user.email="1710998763@qq.com" commit -m "feat(cli): doctor 子命令(渲染 ✓/○/✗ + 硬问题非零退出)"
```
...
git -c user.name="BolynWang" -c user.email="1710998763@qq.com" commit -m "docs(w3b3): doctor 用法 + CHANGELOG"
```
...
```bash
git add -u
git -c user.name="BolynWang" -c user.email="1710998763@qq.com" commit -m "chore(w3b3): lint/type 收尾"
```
