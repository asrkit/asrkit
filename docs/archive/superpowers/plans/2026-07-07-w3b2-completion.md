...
git -c user.name="BolynWang" -c user.email="lm2039136@gmail.com" commit -m "feat(cli): list --ids(裸 id,懒算 inst)"
```
...
git -c user.name="BolynWang" -c user.email="lm2039136@gmail.com" commit -m "feat(completion): asrkit completion <bash|zsh|fish>(限位置 model 补全,zsh 双模)"
```
...
git -c user.name="BolynWang" -c user.email="lm2039136@gmail.com" commit -m "docs(w3b2): shell 补全安装说明 + CHANGELOG"
```
...
```bash
git add -u
git -c user.name="BolynWang" -c user.email="lm2039136@gmail.com" commit -m "chore(w3b2): lint/type 收尾"
```
