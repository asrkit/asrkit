...
当前未发布源码另提供未来 `asrkit-cloud` 使用的 cloud-only 构建入口。它复用相同 HTTP server,但进程只注册 10 个内置云模型,不加载本地 adapter、用户模型或第三方插件。完整 Python wheel **不会**安装 `asrkit-cloud` 命令,从而可与未来独立产物并存；冻结完成前可用下面的模块命令做开发验证:
...
| `ASRKIT_GATEWAY_TOKEN` | 当前源码 `asrkit-cloud` 的 Bearer token；embedded 必填且至少 32 字符 |
