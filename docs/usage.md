...
asrkit transcribe a.wav -m sherpa/whisper --format srt -o a.srt   # 出字幕
...
```bash
asrkit stream sherpa/paraformer-online a.wav          # live 进度 stderr,最终文本 stdout
asrkit stream sherpa/paraformer-online a.wav > out.txt  # 只截获最终文本
asrkit stream sherpa/paraformer-online a.m4a --convert  # opt-in 解码/重采样
```
...
```bash
asrkit stream sherpa/paraformer-online --mic          # 实时麦克风流式,Ctrl-C 停
asrkit stream sherpa/paraformer-online --mic --device 1   # 指定设备(索引或名称子串)
```
...
```bash
asrkit run sherpa/paraformer a.wav -v      # INFO:看到 _http 重试等
asrkit run sherpa/paraformer a.wav -vv     # DEBUG:额外打印 model/metrics
...
curl -N http://127.0.0.1:11435/v1/audio/transcriptions \
  -F model=sherpa/paraformer-online \
  -F stream=true \
  -F file=@a.wav
```
