...
> 状态:brainstorming 完成、用户批准、**Codex(gpt-5.5)评审采纳 1 项 → v2**,待实现计划。
>
> **v2 修订**(Codex):端点检测再传 `rule3_min_utterance_length=300.0`。sherpa 默认 `rule3=20.0` 会在**连续说话 ~20s** 时强制切段/reset(与静音无关),切断解码上下文;上游麦克风端点示例用 300 实质禁用该规则,让端点由静音(rule1/rule2)驱动。其余 4 点 Codex 确认无误(capture-before-reset 顺序、join、kwarg 对 sherpa≥1.13.3 有效、契约兼容、reset 后 flush)。
...
> `rule3_min_utterance_length=300.0`(v2):实质禁用"长度切段",端点由静音(rule1/rule2 默认 2.4s/1.2s 尾静音)驱动,避免连续说话 20s 被硬切。
