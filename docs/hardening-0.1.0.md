...
### H-14 [P0] PyPI 恢复码移出仓库树 · `TODO`（Codex 上调 P0：凭据安全）
`ASR全景调研/PyPI-Recovery-Codes-*.txt` 移进密码管理器并从磁盘删除。发布用 Trusted Publisher / 项目级 scoped token。
...
### H-17 [P1] 模型许可证标注 · `TODO`（Codex 上调 P1）
`models_local.py` 全部 `license=None` → 补齐；研究型模型显著标注"仅研究用途"。差异化卖点。
...
**P0（阻断）**
- [ ] D-1 内核零处理 · D-2 云端 samples=None 原样直传 · D-3 warnings 不静默截断
- [ ] H-01 tar 安全解压（全类型验收）
- [ ] H-14 恢复码移出仓库树
- [ ] H-19 sdist/wheel 内容审计（无敏感文件）

**P1**
- [ ] H-02 pull 原子 + install_files 校验
- [ ] H-03a 下载超时 · H-03b sha256 校验
- [ ] H-04 CLI 异常兜底（Codex 实测点）
- [ ] H-05 环境变量 key
- [ ] H-09 sherpa ≥1.13.3 · H-10 README 重写 · H-11 分类器 Beta
- [ ] H-13 冒烟测试 + CI 绿 · H-16 文档一致 · H-17 许可证标注

**构建/安装验证（Codex）**
- [ ] `python -m build` + `twine check dist/*` 通过
- [ ] 新 venv 装 wheel 后 `asrkit list` 正常
- [ ] `pip install .[cloud]` / `.[local]` / `.[all]` 分别可用
- [ ] CLI 拼错模型名无 traceback
